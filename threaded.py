...
def main():
    main_agent = None
    agents, envs = [], []
    hs = HSearchEnv(args.agent, gpu_split=args.gpu_split)
    ...
    for i in range(args.workers):
        ...
        env = BitcoinEnv(name=name, hypers=flat, write_graph=write_graph)
        envs.append(env)
    ...
    for e in envs:
        hs.run_finished(e.episode_results['rewards'])
        e.close()
