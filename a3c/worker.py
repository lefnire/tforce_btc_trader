        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                if episode_count % 1 == 0 and not episode_count % 100 == 0 and not self.is_test:
                    ...
                    mean_total = np.mean(self.episode_totals[-1])
                    summary = tf.Summary()
                    summary.value.add(tag='Perf/Reward', simple_value=float(mean_reward))
                    summary.value.add(tag='Perf/Total', simple_value=float(mean_total))
                ...
                if self.name == 'worker_0':
                    ...
                    print("{} Reward:{} ${} Actions:{}".format(
                        episode_count,
                        round(episode_reward),
                        round(self.episode_totals[-1]),
                        self.env.action_counter
                    ))
