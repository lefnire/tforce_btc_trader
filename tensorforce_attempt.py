...
agent = DQNAgent(config=Configuration(
    ...
    states=dict(shape=(150), type='float'),
    actions=dict(continuous=False, num_actions=3),
    network=layered_network_builder([
        dict(type='lstm', size=64, summary_level=0),
        dict(type='lstm', size=64, summary_level=1)
    ])
))
...
print("Learning finished. Total episodes: {ep}. Average reward of last 100 episodes: {ar}.".format(
    ep=runner.episode, ar=np.mean(runner.episode_rewards[-100:])))