...
if __name__ == "__main__":
    ...
    env = BitcoinEnv(agent_type='A2CAgent', agent_name='A2CAgent')
    # get size of state and action from environment
    state_size = env.num_features()
    action_size = env.actions['num_actions']
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                print(e, "  score:", round(score), " total:", round(env.cash+env.value), " actions:", env.action_counter)

                # if the mean of scores of last 10 episode is bigger than 490
                # stop training
                # if np.mean(scores[-min(10, len(scores)):]) > 2100:
                #     sys.exit()
