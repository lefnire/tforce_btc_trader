...
class MlpPolicy(object):
    ...
    def lstm(self, x, size, name):
        ...
        self.c_in = U.get_placeholder(name="c_in", dtype=tf.float32, shape=[None, cell.state_size.c])
        self.h_in = U.get_placeholder(name="h_in", dtype=tf.float32, shape=[None, cell.state_size.h])
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        self._act = U.function(
            [stochastic, ob, self.c_in, self.h_in],
            [ac, self.vpred, self.rnn_next],
        )

    def act(self, stochastic, ob, rnn_state):
        ac1, vpred1, rnn_next = self._act(stochastic, ob[None], rnn_state[0], rnn_state[1])
